 ...
 * - POST /api/rentals requires customer_id and car_id. This is a client
 *   component, and `requireUser()` (from @/lib/IAM/validators) is a
 *   server-side check — it can't run here. Instead, `customerId` and
 *   `carId` are passed in as PROPS by the server component that renders
 *   this page (see page.tsx), which calls requireUser() and reads the
 *   route's `params` itself, then hands both values down. Neither is
 *   shown or editable in the form.
 * - TripMap's `route` prop is optional: with no rental loaded yet (e.g. on
 *   first page load), the map still renders — just centered on a default
 *   Manhattan, NYC view — instead of a blank/text-only placeholder.
 * - handleCreateRental validates every field the backend requires
 *   (customer_id, car_id, start_time, end_time, pickup_location,
 *   pickup_latitude, pickup_longitude, dropoff_location,
 *   dropoff_latitude, dropoff_longitude) using Number.isFinite rather than
 *   plain truthiness checks. A truthy check (`!pickup_latitude`) would
 *   wrongly reject a valid coordinate of exactly 0 (e.g. on the equator or
...
// Passed down by the server component that renders this page — see page.tsx.
export interface ServicesProps {
  /** Logged-in user's id, resolved server-side via requireUser(). */
  customerId: string;
  /** This page's car, resolved server-side from the route's params. */
  carId: string;
}
...
export default function Services({ customerId, carId }: ServicesProps) {
  ...
  // Pre-filled with an example NYC route on first load — same effect as a
  // manual search, which is a reliable way to confirm the map is painting
  // correctly, and it doubles as a placeholder for the create-rental form.
  const [fromPoint, setFromPoint] = useState<SearchPoint | null>({
    point: { lat: 40.758, lng: -73.9855 },
    label: 'Times Square, Manhattan, NY, USA',
  });
  const [toPoint, setToPoint] = useState<SearchPoint | null>({
    point: { lat: 40.6413, lng: -73.7781 },
    label: 'JFK Airport, Queens, NY, USA',
  });
  const [routeInfo, setRouteInfo] = useState<RouteInfo | null>(null);
  const [routeLoading, setRouteLoading] = useState(false);

  // Create-rental form fields. customer_id / car_id come in as props (see
  // ServicesProps above) — resolved server-side, not editable here.
  ...
  // ── Create a new rental, validated field-by-field against exactly what
  //    the backend requires (see the `if (!customer_id || ...)` check).
  //    Number.isFinite is used instead of plain truthiness so a real
  //    coordinate of 0 (equator / prime meridian) isn't wrongly rejected,
  //    and each failure names the specific missing/invalid field instead
  //    of one generic message. ──────────────────────────────────────────
  const handleCreateRental = async () => {
    setCreateError(null);
    setCreatedRental(null);

    // customer_id — resolved server-side and passed in as a prop; must be
    // a real positive number, not just "truthy" (an empty-string prop
    // would otherwise silently become NaN downstream).
    const customerIdNum = Number(customerId);
    if (!customerId || !Number.isFinite(customerIdNum) || customerIdNum <= 0) {
      setCreateError('Missing or invalid customer_id — you need to be signed in to create a rental.');
      return;
    }

    // car_id — resolved server-side from the route params.
    const carIdNum = Number(carId);
    if (!carId || !Number.isFinite(carIdNum) || carIdNum <= 0) {
      setCreateError("Missing or invalid car_id — this page must be opened from a car's showroom page.");
      return;
    }

    // pickup — location label plus lat/lng, all required.
    if (!fromPoint || !fromPoint.label.trim()) {
      setCreateError('Missing pickup_location — pick a "From" location first.');
      return;
    }
    if (!Number.isFinite(fromPoint.point.lat) || !Number.isFinite(fromPoint.point.lng)) {
      setCreateError('Missing or invalid pickup_latitude/pickup_longitude — re-select the "From" location.');
      return;
    }

    // dropoff — location label plus lat/lng, all required.
    if (!toPoint || !toPoint.label.trim()) {
      setCreateError('Missing dropoff_location — pick a "To" location first.');
      return;
    }
    if (!Number.isFinite(toPoint.point.lat) || !Number.isFinite(toPoint.point.lng)) {
      setCreateError('Missing or invalid dropoff_latitude/dropoff_longitude — re-select the "To" location.');
      return;
    }

    // start_time / end_time — required, must parse to valid dates, and
    // end must be after start.
    if (!startTime) {
      setCreateError('Missing start_time — set a start time.');
      return;
    }
    if (!endTime) {
      setCreateError('Missing end_time — set an end time.');
      return;
    }
    const startDate = new Date(startTime);
    const endDate = new Date(endTime);
    if (Number.isNaN(startDate.getTime())) {
      setCreateError('Invalid start_time — pick a valid start date/time.');
      return;
    }
    if (Number.isNaN(endDate.getTime())) {
      setCreateError('Invalid end_time — pick a valid end date/time.');
      return;
    }
    if (endDate.getTime() <= startDate.getTime()) {
      setCreateError('end_time must be after start_time.');
      return;
    }

    // Everything the backend requires is confirmed present and valid —
    // safe to build the payload now.
  ...
  // `mapRoute` is null until a rental is loaded — TripMap handles that
  // gracefully by falling back to a default Manhattan, NYC view.
  const mapRoute: TripRoute | null = rental ? buildMapRoute(rental) : null;
        ...
        <div className="mx-auto grid h-full max-w-[1920px] grid-cols-[76px_260px_1fr]">
          ...
          <section className="relative h-full min-h-0 overflow-hidden bg-black">
            {/*
              ...
              TripMap is ALWAYS mounted — even before a rental has loaded — so the
              user sees a live map (defaulting to Manhattan, NYC) on first page load
              instead of a blank/text-only placeholder. Once a rental loads, `mapRoute`
              flips from null to a real route and the map flies to it.
            */}
                        ...
                        <div className="grid grid-cols-2 gap-2.5">
                          <div className="flex flex-col gap-1.5">
                            ...
                            <input
                              ...
                              className="rounded-lg border border-white/10 bg-black/30 px-2.5 py-2 text-[12.5px] text-[#F5F5F5] outline-none focus:border-[#8C7CFF] [color-scheme:dark]"
                            />
                          </div>
                          <div className="flex flex-col gap-1.5">
